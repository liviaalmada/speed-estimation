...
import br.ufc.arida.mapmatching.TrajectoryPoint;
import br.ufc.arida.mapmatching.Edge;
import br.ufc.arida.mapmatching.TrajectoryAsSet;
...
public class TrajectoryPointDAO {
	...
	public void insert(ArrayList<TrajectoryPoint> points) {
		try {
			...
			PreparedStatement prepareStatement = connection
					.prepareStatement("insert into taxi_junho(id, latitude, longitude, date_time) values(?,?,?,?)");
	...
	public ArrayList<TrajectoryAsSet> readTrajectoriesAsSet(String tableName, Timestamp startDate, Timestamp endDate)
	...
	public Map<Integer, List<GPXEntry>> readTrajectoriesAsGPXEntriesTSDatabase(String tableName, Timestamp startDate,
		...
		while (result.next()) {
			...
			list.add(new GPXEntry(new GHPoint(result.getDouble("latitude"), result.getDouble("longitude")),
	...
	public void updateMapMatching(Vector<TrajectoryAsSet> objects, String trajTableName)
			throws ClassNotFoundException, SQLException, IOException {
		Connection connection = ConnectionJDBC.getConnection();
	...
	public ArrayList<TrajectoryAsSet> readTrajectoriesAsSetWithEdges(String tableName, Timestamp startDate,
			Timestamp endDate) throws ClassNotFoundException, SQLException, IOException {
		Connection connection = ConnectionJDBC.getConnection();
		String strQuery = "select * from " + tableName
				+ " where date_time between ? and ? and edge_id is not null  order by id, date_time";
		PreparedStatement query = connection.prepareStatement(strQuery);
		ArrayList<Long> time_array = new ArrayList<>();
		ArrayList<Coordinate> coord_array = new ArrayList<>();
